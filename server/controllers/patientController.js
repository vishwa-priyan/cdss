...
export async function list(req, res) {
  try {
    ...
    const disease = req.query.disease?.trim() || null;
    ...
    const [countRow] = await query(
      `SELECT COUNT(DISTINCT p.id) AS total FROM patients p WHERE ${where}`,
      params
    );
    ...
    const rows = await query(paginatedSql, params);
  ...
  try {
    ...
    const [insertPatient] = await conn.execute(
      `INSERT INTO patients
       (name, age, gender, contact, chief_complaint,
    ...
    if (symptoms) {
      await conn.execute(
        'INSERT INTO symptoms (encounter_id, symptom_text) VALUES (?, ?)',
        [encounterId, symptoms]
      );
    }
...
export async function getById(req, res) {
  ...
  for (const enc of encounters) {
    enc.vitals = await query(
      'SELECT * FROM vitals WHERE encounter_id = ? ORDER BY recorded_at DESC',
      [enc.id]
    );

    enc.symptoms = await query(
      'SELECT * FROM symptoms WHERE encounter_id = ?',
      [enc.id]
    );

    enc.lab_reports = await query(
      'SELECT * FROM lab_reports WHERE encounter_id = ?',
      [enc.id]
    );

    const [ar] = await query(
      'SELECT * FROM ai_results WHERE encounter_id = ?',
      [enc.id]
    );
